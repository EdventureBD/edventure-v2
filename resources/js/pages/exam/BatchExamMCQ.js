...
const BatchExamMCQ = ({ questions, batch, exam }) => {
    ...
    const submitExam = async (e) => {
        ...
        if (state.answers.length < questions.length) {
            ...
            const res = await axios.post(url, { answers: state.answers })
                .then(response => {
    ...
    questions.map((qus, qindex) => {
        ...
        // console.log(sanswer);
        questionSummary.push(<div class={`single-qus-summary ${sanswer ? "bg-red" : "bg-green"} bradius-15 c-point`}>{qindex + 1}</div>)
    })
    ...
    return (<div className="batch-exam">
        <div className="container">
            ...
            <form action={"/batch/" + batch.slug + "/" + exam.slug + "/result"} method="post" id="exam-form" onSubmit={submitExam}>
                ...
                <button className="btn text-xxsm fw-800 text-white bg-purple px-4 py-2 mt-3" >Submit</button>
            </form>
        </div>
    </div>)
}
