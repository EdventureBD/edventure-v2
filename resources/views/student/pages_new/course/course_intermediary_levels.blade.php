            ...
            <div class="text-center @if($intermediary_levels->count()>=7) course-category-js @endif">
               @foreach($intermediary_levels as $level)
                     @if($level->slug==$selected_intermediary_level->slug)
                        <a href="{{ route('intermediary-level-preview', [$selected_category->slug, $level->slug]) }}"  class="mb-3 d-inline-block course-category-single-js btn fw-800 text-xxsm text-white
                           mx-1 bradius-15 bshadow-medium bg-purple px-4">{{$level->title}}</a>
                     @else
                        <a href="{{ route('intermediary-level-preview', [$selected_category->slug, $level->slug]) }}"  class="mb-3  d-inline-block course-category-single-js btn fw-800 text-xxsm text-purple
                  ...
                  @foreach ($courses as $course)
                     <div class="col-md-3 mb-4">                        
                        <div class="single-exam text-center mx-auto p-4 mb-md-0" style="background-image: url({{asset($course->banner)}});">
                              ...
                              <a href=""  class="btn btn-outline text-purple mt-2">See Courses</a>
                              </div>
                        </div>
                     </div>
                  @endforeach
